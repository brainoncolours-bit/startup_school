...
const OverloadMatrix = () => {
  ...
  return (
    <div ref={containerRef} className="bg-[#0a0a0a] text-white font-black overflow-hidden selection:bg-[#e72132]">
      
      {/* 1. THE "SYSTEM BOOT" HERO */}
      <section className="h-screen relative flex items-center justify-center overflow-hidden">
        ...
        {/* Rapid Pulsing Overlay */}
        <motion.div 
          animate={{ opacity: [0.1, 0.3, 0.1] }}
          transition={{ duration: 0.1, repeat: Infinity }}
          className="absolute inset-0 bg-[#a5cb3a]/5"
        />
        ...
        <div className="z-10 text-center relative">
          ...
          >
            <div className="px-4 py-1 bg-[#e72132] text-white font-mono text-[10px] tracking-[0.5em] animate-pulse">
              CRITICAL ACCESS ONLY
            </div>
          </motion.div>
          <h1 className="text-[18vw] leading-[0.7] italic uppercase tracking-tighter mix-blend-difference">
            RADICAL <br /> <span className="text-[#a5cb3a]">ALPHA.</span>
          </h1>
          <div className="mt-10 flex items-center justify-center gap-4 text-[#a5cb3a]">
            <Activity size={20} />
            <span className="font-mono text-xs tracking-widest">ENROLLMENT_SEQUENCE_START</span>
          </div>
        </div>

        {/* Floating Technical HUD */}
        <div className="absolute bottom-10 left-10 text-[10px] font-mono text-white/30 space-y-1">
...
const ExtremeRow = ({ num, title, desc, accent, reverse }) => {
  ...
  return (
    <section ref={ref} className={`min-h-screen flex items-center p-6 transition-colors duration-700 ${isInView ? 'bg-white text-black' : 'bg-black text-white'}`}>
      <div className={`max-w-7xl mx-auto w-full flex flex-col lg:flex-row items-center gap-20 ${reverse ? 'lg:flex-row-reverse' : ''}`}>
        ...
        <div className="flex-1 space-y-8">
          ...
          <p className="text-2xl font-bold italic leading-tight opacity-70">
            {desc}
          </p>
        ...
        <div className="flex-1 w-full relative">
           ...
           />
           <div className="aspect-[4/5] bg-neutral-900 border-[10px] border-current flex items-center justify-center overflow-hidden">
              <Cpu size={120} className="text-white animate-pulse" />
              {/* This would be a high-glitch video or abstract texture */}
